locals {
  name_original_tags           = join(var.delimiter, compact(concat(list(var.name, var.namespace, var.stage), var.attributes)))
  environment_original_tags    = join(var.delimiter, compact(concat(list(var.namespace, var.stage))))
